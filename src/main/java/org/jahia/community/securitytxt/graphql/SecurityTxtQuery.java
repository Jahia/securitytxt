...
@GraphQLName("SecurityTxtQuery")
@GraphQLDescription("security.txt queries")
public class SecurityTxtQuery {

    private static final Logger LOGGER = LoggerFactory.getLogger(SecurityTxtQuery.class);
            ...
            return JCRTemplate.getInstance().doExecuteWithSystemSession(new JCRCallback<GqlSecurityTxt>() {
                ...
                public GqlSecurityTxt doInJCR(JCRSessionWrapper session) throws RepositoryException {
                    ...
                    return new GqlSecurityTxt(
                            ...
                            getStringProp(node, "acknowledgmentsUrl"),
                            getStringProp(node, "canonical"),
                            getRefPropUUID(node, "encryption"),
                            getStringProp(node, "encryptionUrl"),
                            getRefPropUUID(node, "hiring"),
                            getStringProp(node, "hiringUrl"),
                            getRefPropUUID(node, "policy"),
                            getStringProp(node, "policyUrl"),
    ...
    public List<GqlFileItem> getSecurityTxtFiles(
        ...
        return queryItems(siteKey, "files", "jnt:file", searchTerm, "files");
    }
    ...
    public List<GqlFileItem> getSecurityTxtPages(
            @GraphQLName("siteKey") @GraphQLNonNull final String siteKey,
            @GraphQLName("searchTerm") final String searchTerm) {
        return queryItems(siteKey, "home", "jnt:page", searchTerm, "pages");
    }

    private List<GqlFileItem> queryItems(final String siteKey, final String relPath,
                                         final String nodeType, final String searchTerm, final String what) {
        try {
            return JCRTemplate.getInstance().doExecuteWithSystemSession(new JCRCallback<List<GqlFileItem>>() {
                ...
                public List<GqlFileItem> doInJCR(JCRSessionWrapper session) throws RepositoryException {
                    return getSiteItems(session, siteKey, relPath, nodeType, searchTerm);
                }
            });
        } catch (AccessDeniedException e) {
            LOGGER.warn("Access denied getting {} for site {}: {}", what, siteKey, e.getMessage());
            throw new SecurityTxtAccessDeniedException("Access denied: siteAdminSecurityTxt permission required");
        } catch (RepositoryException e) {
            LOGGER.error("Error getting {} for site {}", what, siteKey, e);
